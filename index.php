...
$anuncios = $AnuncioDAO->getAllPaginacion($pagina_actual);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($usuario != null && password_verify($password, $usuario->getPassword())) {
        //Usuario autenticado con éxito, guardamos su información en la sesión
        $_SESSION['usuario'] = $usuario->getNombre();
        $_SESSION['id'] = $usuario->getId();
        $_SESSION['email'] = $usuario->getEmail();
        setcookie("usuario", $usuario->getNombre(), time() + 60 * 60 * 24 * 7, "/");
        setcookie("id", $usuario->getId(), time() + 60 * 60 * 24 * 7, "/");
        setcookie("email", $usuario->getEmail(), time() + 60 * 60 * 24 * 7, "/");
        header('Location: index.php');
        exit();
    } else {
        $errorInicio = 'Credenciales incorrectas. Por favor, inténtalo de nuevo.';
    }
}

// Verificar y renovar las cookies por otros 7 días
if (isset($_COOKIE['usuario']) && isset($_COOKIE['email']) && isset($_COOKIE['id'])) {
    setcookie("usuario", $_COOKIE['usuario'], time() + 60 * 60 * 24 * 7, "/");
    setcookie("id", $_COOKIE['id'], time() + 60 * 60 * 24 * 7, "/");
    setcookie("email", $_COOKIE['email'], time() + 60 * 60 * 24 * 7, "/");
}
?>
...
<body>
    ...
    <div id="menu">
        <ul>
            ...
            <li><?php if (isset($_SESSION['usuario']) && $_SESSION['usuario'] != null) { ?>
        ...
        <?php foreach ($anuncios as $anuncio) : ?>
            <div class="anuncio">
                ...
                <?php if (isset($_SESSION['id'])) {
                    if ($anuncio->getUsuarioId() == $_SESSION['id']) { ?>
                        <span class="icono_borrar"><a href="borrar_anuncio.php?ID=<?= $anuncio->getId() ?>" onclick="return confirmarBorrado();"><i class="fa-solid fa-trash color_gris"></i></a></span>
    ...
    <script>
        function confirmarBorrado() {
            return confirm("¿Estás seguro que deseas borrar este anuncio?");
        }
    </script>
</body>
